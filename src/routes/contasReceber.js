 ...
 *   get:
 *     summary: Lista as contas a receber da loja (ledger manual — fiado, acerto combinado fora do gateway etc.)
 *     tags: [Financeiro]
 *     parameters:
 *       - in: path
 *         name: empresaId
 ...
 * /empresas/{empresaId}/contas-receber:
 *   post:
 *     summary: Registra uma nova conta a receber
 *     tags: [Financeiro]
 *     parameters:
 *       - in: path
 *         name: empresaId
...
router.post('/', asyncHandler(async (req, res) => {
  const { descricao, clienteNome, valor, vencimento, observacoes } = req.body;
  ...
  const conta = await prisma.contaReceber.create({
    data: {
      ...
      clienteNome: clienteNome ? String(clienteNome).trim() : null,
      valor: valorNumero,
...
/**
 * @openapi
 * /empresas/{empresaId}/contas-receber/{id}:
 *   patch:
 *     summary: Atualiza uma conta a receber (dados, ou marcar como recebida/reabrir)
 *     tags: [Financeiro]
 *     parameters:
 *       - in: path
 *         name: empresaId
...
router.patch('/:id', asyncHandler(async (req, res) => {
  const existente = await prisma.contaReceber.findFirst({ where: { id: req.params.id, empresaId: req.params.empresaId } });
  if (!existente) {
    return res.status(404).json({ error: 'Conta não encontrada' });
  }

  const { descricao, clienteNome, valor, vencimento, observacoes, status } = req.body;
  const data = {};
  if (descricao !== undefined) data.descricao = String(descricao).trim();
  if (clienteNome !== undefined) data.clienteNome = clienteNome ? String(clienteNome).trim() : null;
  if (valor !== undefined) {
    const valorNumero = Number(valor);
    if (!Number.isFinite(valorNumero) || valorNumero <= 0) {
      return res.status(400).json({ error: 'Campo "valor" deve ser maior que zero' });
  ...
  if (status !== undefined) {
    ...
    data.status = status;
    data.recebidoEm = status === 'PAGO' ? new Date() : null;
  }

  const conta = await prisma.contaReceber.update({ where: { id: req.params.id }, data });
  res.json(conta);
}));
